...
class mariaalejandraydiegoController extends Controller
{

	private $_mariaalejandraydiego;
	private $_dt;
	private $_key;
	...
	public function __construct()
	{
		...
		$this->_mariaalejandraydiego = $this->loadModel('couple');
	...
	public function guardarMensaje()
	{
		...
		echo json_encode($registro = $this->_mariaalejandraydiego->guardarmensaje(
	...
	public function guardarmensajeMonto()
	{
		...
		echo json_encode($registro = $this->_mariaalejandraydiego->guardarmensajeMonto(
			$messageId,
	...
	public function g_ao()
	{
		...
		echo json_encode($this->_mariaalejandraydiego->g_ao($this->getTexto("ao")));
	...
	public function obsequio($codigo)
	{
		...
		$ps_k = $this->_mariaalejandraydiego->keysEmp($this->_key);
		$mensaje = $this->_mariaalejandraydiego->buscarMensaje($codigo);
	...
	public function estado($codigo)
	{
		...
		if ($formAnswer['kr-answer']['orderStatus'] == 'PAID') {
			...
			$this->_mariaalejandraydiego->cambiarMensajeEstado($codigo, $uuid, $hash, $formAnswer['kr-answer']['orderStatus']);
	...
	{
		echo json_encode($this->_mariaalejandraydiego->enviarCorreo($codigo));
	}
	...
	public function lista($codigo)
	{
		if ($codigo === "pdcgb") {
			...
			$this->redireccionar('/mariaalejandraydiego');
		}
	}
