...
class julissayrubenController extends Controller
{

	private $_julissayruben;
	private $_dt;
	private $_key;
	...
	public function __construct()
	{
		...
		$this->_julissayruben = $this->loadModel('couple');
	...
	{
		$this->_view->assign('titulo', 'Julissa y Rubén');

		$ps_k = $this->_julissayruben->keysEmp($this->_key);
	...
	public function guardarMensaje()
	{
		...
		echo json_encode($registro = $this->_julissayruben->guardarmensaje(
	...
	public function guardarmensajeMonto()
	{
		...
		echo json_encode($registro = $this->_julissayruben->guardarmensajeMonto(
			$messageId,
	...
	public function g_ao()
	{
		...
		echo json_encode($this->_julissayruben->g_ao($this->getTexto("ao")));
	...
	public function obsequio($codigo)
	{
		...
		$ps_k = $this->_julissayruben->keysEmp($this->_key);
		$mensaje = $this->_julissayruben->buscarMensaje($codigo);
	...
	public function estado($codigo)
	{
		...
		if ($formAnswer['kr-answer']['orderStatus'] == 'PAID') {
			...
			$this->_julissayruben->cambiarMensajeEstado($codigo, $uuid, $hash, $formAnswer['kr-answer']['orderStatus']);
	...
	{
		echo json_encode($this->_julissayruben->enviarCorreo($codigo));
	}
	...
	public function lista($codigo)
	{
		if ($codigo === "pdcgb") {
			...
			$this->redireccionar('/julissayruben');
		}
	}
