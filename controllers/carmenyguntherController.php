...
class carmenyguntherController extends Controller
{

	private $_carmenygunther;
	private $_dt;
	private $_key;
	...
	public function __construct()
	{
		...
		$this->_carmenygunther = $this->loadModel('couple');
		$this->_obsequio = $this->loadModel('obsequio');
		$this->_index = $this->loadModel('index');
		$this->_dt = $this->loadModel('dataTable');

		$this->_key = 10; // Carmen y Gunther
	}

	public function index()
	{
		$this->_view->assign('titulo', 'Carmen y Gunther');

		$ps_k = $this->_carmenygunther->keysEmp($this->_key);
	...
	public function guardarMensaje()
	{
		...
		echo json_encode($registro = $this->_carmenygunther->guardarmensaje(
	...
	public function guardarmensajeMonto()
	{
		...
		echo json_encode($registro = $this->_carmenygunther->guardarmensajeMonto(
			$messageId,
	...
	public function g_ao()
	{
		...
		echo json_encode($this->_carmenygunther->g_ao($this->getTexto("ao")));
	...
	public function obsequio($codigo)
	{
		...
		$ps_k = $this->_carmenygunther->keysEmp($this->_key);
		$mensaje = $this->_carmenygunther->buscarMensaje($codigo);
	...
	public function estado($codigo)
	{
		...
		if ($formAnswer['kr-answer']['orderStatus'] == 'PAID') {
			...
			$this->_carmenygunther->cambiarMensajeEstado($codigo, $uuid, $hash, $formAnswer['kr-answer']['orderStatus']);
	...
	{
		echo json_encode($this->_carmenygunther->enviarCorreo($codigo));
	}
	...
	public function lista($codigo)
	{
		if ($codigo === "pdcgb") {
			...
			$this->redireccionar('/carmenygunther');
		}
	}
