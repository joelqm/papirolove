...
class mayteyandreeController extends Controller
{

	private $_mayteyandree;
	private $_dt;
	private $_key;
	...
	public function __construct()
	{
		...
		$this->_mayteyandree = $this->loadModel('couple');
		$this->_obsequio = $this->loadModel('obsequio');
		$this->_index = $this->loadModel('index');
		$this->_dt = $this->loadModel('dataTable');

		// TODO: actualizar al ID real (tbl_sede) de Mayte y Andree
		$this->_key = 7;
	}

	public function index()
	{
		$this->_view->assign('titulo', 'Mayte y Andree');

		$ps_k = $this->_mayteyandree->keysEmp($this->_key);
	...
	public function guardarMensaje()
	{
		...
		echo json_encode($registro = $this->_mayteyandree->guardarmensaje(
	...
	public function guardarmensajeMonto()
	{
		...
		echo json_encode($registro = $this->_mayteyandree->guardarmensajeMonto(
			$messageId,
	...
	public function g_ao()
	{
		...
		echo json_encode($this->_mayteyandree->g_ao($this->getTexto("ao")));
	...
	public function obsequio($codigo)
	{
		...
		$ps_k = $this->_mayteyandree->keysEmp($this->_key);
		$mensaje = $this->_mayteyandree->buscarMensaje($codigo);
	...
	public function estado($codigo)
	{
		...
		if ($formAnswer['kr-answer']['orderStatus'] == 'PAID') {
			...
			$this->_mayteyandree->cambiarMensajeEstado($codigo, $uuid, $hash, $formAnswer['kr-answer']['orderStatus']);
	...
	{
		echo json_encode($this->_mayteyandree->enviarCorreo($codigo));
	}
	...
	public function lista($codigo)
	{
		if ($codigo === "pdcgb") {
			...
			$this->redireccionar('/mayteyandree');
		}
	}
