...
class jesykaygustavoController extends Controller
{
	...
	public function guardarMensaje()
	{
		...
		echo json_encode($registro = $this->_jesykaygustavo->guardarmensaje(
	...
	public function guardarmensajeMonto()
	{
		...
		echo json_encode($registro = $this->_jesykaygustavo->guardarmensajeMonto(
			$messageId,
	...
	public function g_ao()
	{
		...
		echo json_encode($this->_jesykaygustavo->g_ao($this->getTexto("ao")));
	...
	public function obsequio($codigo)
	{
		...
		$ps_k = $this->_jesykaygustavo->keysEmp($this->_key);
		$mensaje = $this->_jesykaygustavo->buscarMensaje($codigo);
	...
	public function estado($codigo)
	{
		...
		if ($formAnswer['kr-answer']['orderStatus'] == 'PAID') {
			...
			$this->_jesykaygustavo->cambiarMensajeEstado($codigo, $uuid, $hash, $formAnswer['kr-answer']['orderStatus']);
	...
	{
		echo json_encode($this->_jesykaygustavo->enviarCorreo($codigo));
	}
	...
	public function lista($codigo)
	{
		if ($codigo === "pdcgb") {
			...
			$this->redireccionar('/jesykaygustavo');
		}
	}
