...
class camilaydiegoController extends Controller
{

	private $_camilaydiego;
	private $_dt;
	private $_key;
	...
	public function guardarMensaje()
	{
		...
		echo json_encode($registro = $this->_camilaydiego->guardarmensaje(
	...
	public function guardarmensajeMonto()
	{
		...
		echo json_encode($registro = $this->_camilaydiego->guardarmensajeMonto(
			$messageId,
	...
	public function g_ao()
	{
		...
		echo json_encode($this->_camilaydiego->g_ao($this->getTexto("ao")));
	...
	public function estado($codigo)
	{
		...
		if ($formAnswer['kr-answer']['orderStatus'] == 'PAID') {
			...
			$this->_camilaydiego->cambiarMensajeEstado($codigo, $uuid, $hash, $formAnswer['kr-answer']['orderStatus']);
	...
	{
		echo json_encode($this->_camilaydiego->enviarCorreo($codigo));
	}
	...
	public function lista($codigo)
	{
		if ($codigo === "pdcgb") {
			...
			$this->redireccionar('/camilaydiego');
		}
	}
