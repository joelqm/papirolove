...
class flaviayanibalController extends Controller
{

	private $_flaviayanibal;
	private $_dt;
	private $_key;
	...
	public function __construct()
	{
		...
		$this->_flaviayanibal = $this->loadModel('couple');
	...
	{
		$this->_view->assign('titulo', 'Flavia y Aníbal');
	...
	public function guardarMensaje()
	{
		...
		echo json_encode($registro = $this->_flaviayanibal->guardarmensaje(
	...
	public function guardarmensajeMonto()
	{
		...
		echo json_encode($registro = $this->_flaviayanibal->guardarmensajeMonto(
			$messageId,
	...
	public function g_ao()
	{
		...
		echo json_encode($this->_flaviayanibal->g_ao($this->getTexto("ao")));
	...
	public function obsequio($codigo)
	{
		...
		$ps_k = $this->_flaviayanibal->keysEmp($this->_key);
		$mensaje = $this->_flaviayanibal->buscarMensaje($codigo);
	...
	public function estado($codigo)
	{
		...
		if ($formAnswer['kr-answer']['orderStatus'] == 'PAID') {
			...
			$this->_flaviayanibal->cambiarMensajeEstado($codigo, $uuid, $hash, $formAnswer['kr-answer']['orderStatus']);
	...
	{
		echo json_encode($this->_flaviayanibal->enviarCorreo($codigo));
	}
	...
	public function lista($codigo)
	{
		if ($codigo === "pdcgb") {
			...
			$this->redireccionar('/flaviayanibal');
		}
	}
