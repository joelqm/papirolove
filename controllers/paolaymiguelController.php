...
class paolaymiguelController extends Controller
{

	private $_paolaymiguel;
	private $_dt;
	private $_key;
	private $_obsequio;
	private $_index;
	...
	public function __construct()
	{
		...
		$this->_paolaymiguel = $this->loadModel('couple');
	...
	{
		$this->_view->assign('titulo', 'Paola y Miguel');

		$ps_k = $this->_paolaymiguel->keysEmp($this->_key);
	...
	public function guardarMensaje()
	{
		...
		echo json_encode($registro = $this->_paolaymiguel->guardarmensaje(
	...
	public function guardarmensajeMonto()
	{
		...
		echo json_encode($registro = $this->_paolaymiguel->guardarmensajeMonto(
			$messageId,
	...
	public function g_ao()
	{
		...
		echo json_encode($this->_paolaymiguel->g_ao($this->getTexto("ao")));
	...
	public function obsequio($codigo)
	{
		...
		$ps_k = $this->_paolaymiguel->keysEmp($this->_key);
		$mensaje = $this->_paolaymiguel->buscarMensaje($codigo);
	...
	public function estado($codigo)
	{
		...
		if ($formAnswer['kr-answer']['orderStatus'] == 'PAID') {
			...
			$this->_paolaymiguel->cambiarMensajeEstado($codigo, $uuid, $hash, $formAnswer['kr-answer']['orderStatus']);
	...
	{
		echo json_encode($this->_paolaymiguel->enviarCorreo($codigo));
	}
	...
	public function lista($codigo)
	{
		if ($codigo === "pdcgb") {
			...
			$this->redireccionar('/paolaymiguel');
		}
	}
