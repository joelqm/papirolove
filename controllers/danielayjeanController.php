...
class danielayjeanController extends Controller
{

	private $_danielayjean;
	private $_dt;
	private $_key;
	...
	public function __construct()
	{
		...
		$this->_danielayjean = $this->loadModel('couple');
	...
	{
		$this->_view->assign('titulo', 'Daniela y Jean');

		$ps_k = $this->_danielayjean->keysEmp($this->_key);
	...
	public function guardarMensaje()
	{
		...
		echo json_encode($registro = $this->_danielayjean->guardarmensaje(
	...
	public function guardarmensajeMonto()
	{
		...
		echo json_encode($registro = $this->_danielayjean->guardarmensajeMonto(
			$messageId,
	...
	public function g_ao()
	{
		...
		echo json_encode($this->_danielayjean->g_ao($this->getTexto("ao")));
	...
	public function obsequio($codigo)
	{
		...
		$ps_k = $this->_danielayjean->keysEmp($this->_key);
		$mensaje = $this->_danielayjean->buscarMensaje($codigo);
	...
	public function estado($codigo)
	{
		...
		if ($formAnswer['kr-answer']['orderStatus'] == 'PAID') {
			...
			$this->_danielayjean->cambiarMensajeEstado($codigo, $uuid, $hash, $formAnswer['kr-answer']['orderStatus']);
	...
	{
		echo json_encode($this->_danielayjean->enviarCorreo($codigo));
	}
	...
	public function lista($codigo)
	{
		if ($codigo === "pdcgb") {
			...
			$this->redireccionar('/danielayjean');
		}
	}
