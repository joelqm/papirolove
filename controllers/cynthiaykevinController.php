...
class cynthiaykevinController extends Controller
{

	private $_cynthiaykevin;
	private $_dt;
	private $_key;
	...
	public function __construct()
	{
		...
		$this->_cynthiaykevin = $this->loadModel('couple');
	...
	{
		$this->_view->assign('titulo', 'Cynthia y Kevin');

		$ps_k = $this->_cynthiaykevin->keysEmp($this->_key);
	...
	public function guardarMensaje()
	{
		...
		echo json_encode($registro = $this->_cynthiaykevin->guardarmensaje(
	...
	public function guardarmensajeMonto()
	{
		...
		echo json_encode($registro = $this->_cynthiaykevin->guardarmensajeMonto(
			$messageId,
	...
	public function g_ao()
	{
		...
		echo json_encode($this->_cynthiaykevin->g_ao($this->getTexto("ao")));
	...
	public function obsequio($codigo)
	{
		...
		$ps_k = $this->_cynthiaykevin->keysEmp($this->_key);
		$mensaje = $this->_cynthiaykevin->buscarMensaje($codigo);
	...
	public function estado($codigo)
	{
		...
		if ($formAnswer['kr-answer']['orderStatus'] == 'PAID') {
			...
			$this->_cynthiaykevin->cambiarMensajeEstado($codigo, $uuid, $hash, $formAnswer['kr-answer']['orderStatus']);
	...
	{
		echo json_encode($this->_cynthiaykevin->enviarCorreo($codigo));
	}
