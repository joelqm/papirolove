...
$(document).ready(function () {
  ...
  $("#giftForm").validate({
    ...
    submitHandler: function (form, event) {
      ...
      $.ajax({
        url: `${$("#root").val()}flaviayanibal/guardarMensajeMonto`,
        method: 'POST',
        data: data,
        success: function (response) {
          window.location.href = `${$("#root").val()}flaviayanibal/obsequio/${formData[0].value}`;
          //form.reset();
          //$(".form").hide();
        },
        error: function (err) {
  ...
  $(document).on("click", ".button-free-gift", function () {
    ...
    Swal.fire({
      ...
      confirmButtonColor: '#6D8397', // Color del botón de confirmación
