...
const generateRandom = () => {
  ...
  $.post(
    ruta + "mariaalejandraydiego/g_ao",
    "ao=" + ao,
    function (respuesta) {
      if (respuesta == true) {
        generateRandom();
...
$(document).ready(function () {
  ...
  $("#giftForm").validate({
    ...
    submitHandler: function (form, event) {
      ...
      $.ajax({
        url: `${$("#root").val()}mariaalejandraydiego/guardarMensajeMonto`,
        method: 'POST',
        data: data,
        success: function (response) {
          window.location.href = `${$("#root").val()}mariaalejandraydiego/obsequio/${formData[0].value}`;
          //form.reset();
          //$(".form").hide();
        },
        error: function (err) {
