...
$(document).ready(function () {
  ...
  $("#giftForm").validate({
    ...
    submitHandler: function (form, event) {
      ...
      $.ajax({
        url: `${$("#root").val()}mayteyandree/guardarMensajeMonto`,
        method: 'POST',
        data: data,
        success: function (response) {
          window.location.href = `${$("#root").val()}mayteyandree/obsequio/${formData[0].value}`;
          //form.reset();
          //$(".form").hide();
        },
        error: function (err) {
...
const getGifts = async (categoryId = 0) => {
  try {
    const coupleId = 7; // ID actualizado para 
