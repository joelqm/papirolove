...
$(document).ready(function () {
  ...
  $("#giftForm").validate({
    ...
    submitHandler: function (form, event) {
      ...
      $.ajax({
        url: `${$("#root").val()}paolaymiguel/guardarMensajeMonto`,
        method: 'POST',
        data: data,
        success: function (response) {
          window.location.href = `${$("#root").val()}paolaymiguel/obsequio/${formData[0].value}`;
          //form.reset();
          //$(".form").hide();
        },
        error: function (err) {
...
const getGifts = async (categoryId = 0) => {
  try {
    const coupleId = 4; // ID actualizado para 
